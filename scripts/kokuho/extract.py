...
子ども・子育て支援納付金分について、perCapitaAmountUnder18(18歳未満の被保険者1人あたりの
合計負担額)とperCapitaAmountOver18(18歳以上の被保険者1人あたりの合計負担額)を
読み取ってください。この2つの値はどちらも「その年齢区分の人が実際に負担する均等割の
合計額」であり、年齢による差額(加算額)ではありません。

表が「均等割(全年齢共通の基礎額)」と「18歳以上均等割(18歳以上のみに追加でかかる加算額)」
という2つの列に分かれている場合(例: 均等割1,800円・18歳以上均等割73円という表記)は、
perCapitaAmountUnder18には基礎額をそのまま(1,800)、perCapitaAmountOver18には
基礎額+加算額の合計(1,800+73=1,873)を入れてください。18歳以上均等割の列の数値を
そのままperCapitaAmountOver18に入れてはいけません(それは加算額であって合計額ではないため)。

表に最初から「均等割合計」「18歳以上の合計負担額」のような合計値の列がある場合は、
その合計値をそのままperCapitaAmountOver18に入れてください。

年齢による区別が全く無い場合は、同額を両方に入れてください。
