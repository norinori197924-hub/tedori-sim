...
function employeeTax(input, socialInsuranceTotal) {
  const income = calculateEmployeeIncome(input, rates);
  return { income, tax: calculateIncomeTax(income.totalIncome, input, socialInsuranceTotal, rates) };
}
...
test('子供0人・配偶者なしでも例外にならない', () => {
  assert.doesNotThrow(() => employeeTax(baseInput(), 700000));
});

test('年収が極端に低い場合(100万円)、所得税は0円になる', () => {
  const { tax } = employeeTax(baseInput({ annualIncome: 1000000 }), 100000);
  assert.equal(tax.total, 0);
});

test('年収850万円超・23歳未満の扶養親族がいる場合、所得金額調整控除が反映される', () => {
  const { income } = employeeTax(baseInput({ annualIncome: 9000000, numberOfChildren: 1, childrenAges: [10] }), 900000);
  assert.ok(income.incomeAdjustmentDeduction > 0);
});

test('年収が極端に高い場合(3000万円)でも例外にならず、所得税がかかる', () => {
  const { tax } = employeeTax(baseInput({ annualIncome: 30000000 }), 1500000);
  assert.ok(tax.total > 0);
});

test('所得税額と復興特別所得税の合計が100円未満切り捨てになっている', () => {
  const { tax } = employeeTax(baseInput(), 700000);
  assert.equal(tax.total % 100, 0);
});
